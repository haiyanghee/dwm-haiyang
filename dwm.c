	...
	if ((c = wintoclient(ev->window))) {
		if (ev->value_mask & CWBorderWidth)
			...
			if ((c->x + c->w) > m->mx + m->mw && c->isfloating)
				c->x = m->mx
				       + (m->mw / 2
					  - WIDTH(c) / 2); /* center in x
							      direction */
			if ((c->y + c->h) > m->my + m->mh && c->isfloating)
				c->y = m->my
				       + (m->mh / 2
					  - HEIGHT(c) / 2); /* center in y
							       direction */
...
void drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		drw_text(drw, m->ww - sw - stw, 0, sw, bh, lrpad / 2 - 2, stext,
			 0);
	}
...
void keypress(XEvent *e)
{
	...
	for (i = 0; i < LENGTH(keys); i++)
		if (keysym == keys[i].keysym && ev->type == keys[i].type
...
void updatesystrayiconstate(Client *i, XPropertyEvent *ev)
{
	...
	if (flags & XEMBED_MAPPED && !i->tags) {
		...
		return;
	sendevent(i->win, xatom[Xembed], StructureNotifyMask, CurrentTime, code,
...
void updatesystray(void)
{
	...
	if (!systray) {
		...
		XChangeProperty(
			dpy, systray->win, netatom[NetSystemTrayOrientation],
...
{
	if (selmon->sel != NULL) {
		if (__builtin_popcount(selmon->tagset[selmon->seltags]
				       & TAGMASK)
			    == 1
